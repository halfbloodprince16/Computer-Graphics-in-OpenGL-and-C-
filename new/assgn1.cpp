...
void myDisplay()
{
	...
	else
    {
        ...
        int midx1=(x1+x2)/2,midx2=(x2+x3)/2,midx3=(x3+x4)/2,midx4=(x4+x1)/2;
        int midy1=(y1+y2)/2,midy2=(y2+y3)/2,midy3=(y3+y4)/2,midy4=(y4+y1)/2;
        dda(midx1, midy1, midx2, midy2);
        dda(midx2,midy2,midx3,midy3);
        dda(midx3,midy3,midx4,midy4);
        dda(midx4,midy4,midx1,midy1);

        int mid2x1=(midx1+midx2)/2,mid2x2=(midx2+midx3)/2,mid2x3=(midx3+midx4)/2,mid2x4=(midx4+midx1)/2;
        int mid2y1=(midy1+midy2)/2,mid2y2=(midy2+midy3)/2,mid2y3=(midy3+midy4)/2,mid2y4=(midy4+midy1)/2;
        dda(mid2x1, mid2y1, mid2x2, mid2y2);
        dda(mid2x2,mid2y2,mid2x3,mid2y3);
        dda(mid2x3,mid2y3,mid2x4,mid2y4);
        dda(mid2x4,mid2y4,mid2x1,mid2y1);
    }
    glFlush();
}
...
int main(int argc, char **argv)
{
	...
	cout<<"Enter Diagonal End Bottom most left Points :\n X:-";
	cin>>x1;
	
	cout<<"Enter  Diagonal End Bottom most left Points :\n Y:-";
	cin>>y1;
	
	cout<<"Enter  Diagonal End Top most Right Points :\n Y:-";
	cin>>x3;
